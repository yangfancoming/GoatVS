...
namespace chapter1_6_1 {
    ...
    public class Linq2 {
        ...
        public static void test1() {
            ...
            queryResults.ForEach(x => Debug.Print(x.ToString()));
        }
        ...
        public static void test2() {
            ...
            enumerable.ForEach(x => Debug.Print(x.ToString()));
            Debug.Print("---------演示 take ---------");
            var take = enumerable.Take(5);
            take.ForEach(x => Debug.Print(x.ToString()));

            Debug.Print("---------演示 skip ---------");
            var skip = enumerable.Skip(5);
            skip.ForEach(x => Debug.Print(x.ToString()));
        }
        ...
        public static void test4() {
            ...
            Debug.Print("---------获取两个表的交集  Intersect() ---------" );
            var customersWithOrders = customerIDs.Intersect(orderIDs);
            customersWithOrders.ForEach(x => Debug.Print(x));

            Debug.Print("---------获取两个表的差集 Except()---------" );
            var ordersNoCustomers = orderIDs.Except(customerIDs);
            ordersNoCustomers.ForEach(x => Debug.Print(x));

            // 注意，ID 的输出顺序与它们在顾客和订单列表中的顺序相同，只是删除了重复的项。
            Debug.Print("---------获取两个表的并集 Union()---------" );
            var allCustomerOrderIDs = orderIDs.Union(customerIDs);
            allCustomerOrderIDs.ForEach(x => Debug.Print(x));
        }
